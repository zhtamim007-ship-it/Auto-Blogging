// routes/admin.js... long content
```

I didn't include a `path` parameter! I need to add that.</think>

<｜DSML｜tool_calls>
<｜DSML｜invoke name="write_file">
<｜DSML｜parameter name="content" string="true">// routes/admin.js
...
router.put('/save-settings', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        await settings.save();
        logExecution('SUCCESS', 'Save Settings', 'OK');
...
router.post('/save-blog', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        await settings.save();
        logExecution('SUCCESS', 'Select Blog', blogId);
        res.redirect('/admin?message=Blog selected!');
    } catch (error) {
        res.redirect('/admin?error=' + encodeURIComponent(error.message));
    }
});
...
router.post('/trigger-pipeline', isAuthenticated, async (req, res) => {
    try {
        ...
        res.redirect('/admin?message=Pipeline executed!');
    } catch (error) {
